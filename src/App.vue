<template>
  <div class="app">
    <!-- ========== Header ========== -->
    <header
      class="font-black md:text-6xl sm:text-5xl text-stone-400 pt-20 justify-center flex"
    >
    ...
    <main>
      <form
        class="pt-10 flex search-bar h-full flex-col text-center items-center"
        @submit.prevent="addCoin"
      >
        <input
          placeholder="Search for currency to add..."
          class="bg-zinc-100 shadow-lg max-w-2xl w-3/5 py-3 pl-5 rounded-lg"
          type="search"
          required
          v-model="input"
          @input="showDrop"
        ...
        >
          <DropDown
            ...
            :key="drop.asset_id"
            :drop="drop"
            @drop-clicked="dropClicked"
          />
        </ul>
      </form>
      <div class="notFindMsg flex justify-center">
        <h1 v-show="this.hide == true" class="max-w-2xl w-3/5 text-red-500">
          Coin not found...
        </h1>
      </div>
      <ul
        class="justify-center cards-container gap-10 grid py-20 container mx-auto"
      >
...
export default {
  ...
  setup() {
    ...
    const getCoins = () => {
      let formatedCoins = [];
      const API_REQUEST = process.env.VUE_APP_API_REQUEST;

      axios.get(`${API_REQUEST}`).then((response) => {
    ...
    const pushNewCoin = () => {
      ...
      coinList.value.forEach((element) => {
        if (
          element.name.toUpperCase() == input.value.toUpperCase() ||
          element.asset_id == input.value.toUpperCase()
        ) {
          counter = counter + 1;
